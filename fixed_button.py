#!/usr/bin/env python3
# Odds Apex — Golf (Back & Lay)   v2025‑07‑24   Field Quality FIXED: no double-counting, realistic probabilities
...
# No double-counting field quality!
...
def calculate_score():
    ...
    score = (
        ...
        + (sg_off_tee + sg_approach + sg_putting + sg_around_green) * 0.5
    )

    score -= (shots_diff / math.sqrt(max(holes_left, 1))) * SB_SCALE
    score = float(np.clip(score, 0, 100))  # No field bonus/scale here!

    p_model = max(1 / (1 + math.exp(-a*(score-b))), P_FLOOR)

    # ---------- SG projection (cap & regress) ----------
    ...
    if holes_played:
        ...
        sg_rate_h  = min(sg_so_far / holes_played, 0.12)
        sg_remaining = 0.7*sg_expected_pre + 0.3*sg_rate_h*holes_left

    # Simulation uses field-adjusted n_contenders
    p_sim   = simulate_win_prob(shots_diff, holes_left, sg_remaining, n_contenders)

    # Blend (simulation is weighted more)
    p_final = BLEND_MODEL*p_model + (1-BLEND_MODEL)*p_sim
    p_final = min(max(p_final, P_FLOOR), 0.25)  # Cap at 25% win probability
    ...
    new_row = dict(Name=name, p_model=p_final, Market_p=p_mkt, Edge=edge,
                   FairOdds=fair_bl, LiveOdds=live_odds, EV=ev_back,
                   holes_left=holes_left)

    # overwrite last entry if same golfer
    ...
    print(f"{name:12s} | Model:{p_final*100:6.2f}%  "
          ...
          f"Field Quality: {quality_key} Raw score: {score:.2f} Contenders: {n_contenders}")

# ────────────────── HEDGE PERCENT FUNCTION ────────────────── #

def recommended_hedge_pct(odds, holes_left, direction):
    if odds <= 2.5:  return 0.90 if direction=="lay" else 0.65
    if odds <= 4.0:  return 0.60 if holes_left<27 else 0.50
    if odds <= 8.0:  return 0.60 if direction=="lay" else 0.55
    if odds <= 15.0: return 0.60 if direction=="lay" else 0.55
    if odds <= 30.0: return 0.675 if direction=="lay" else 0.60
    return 0.125

# ────────────────── COMPARISON POP‑UP ────────────────── #

def kelly_back_frac(p, o): return (p - 1/o) / (1 - p)
...
def compare_golfers():
    ...
    # back sizing
    def _back_stake(r):
        if r.Edge*100 < EDGE_FLOOR_BACK: return 0.0
        return max(0.0, KELLY_FRACTION*bankroll*kelly_back_frac(r.p_model,r.LiveOdds))
    df["BackStake"] = df.apply(_back_stake, axis=1)
    df["BackWin"]   = df.BackStake * (df.LiveOdds - 1)
    df["BackHedgePct"] = df.apply(lambda r:
        recommended_hedge_pct(r.LiveOdds,r.holes_left,"back") if r.BackStake>0 else 0, axis=1)
    df["BackHedgeStake"] = df.BackStake * df.BackHedgePct
    df["BackHedgePctDisplay"] = df.BackHedgePct.apply(lambda x:f"{int(x*100)}%")
    ...
    cols = ["Name","p_model","Market_p","Edge","FairOdds","LiveOdds",
            ...
            "RankProb","RankEdge"]
    headers = {"p_model":"Model %","Market_p":"Mkt %","Edge":"Edge %",
               "FairOdds":"Fair","LiveOdds":"Odds","BackStake":"Back Stake £",
               "BackWin":"Back Win £","BackHedgeStake":"Back Hedge £",
               "BackHedgePctDisplay":"Back Hedge %","LayStake":"Lay Stake £",
               "LayLiability":"Lay Liab £","LayHedgeStake":"Lay Hedge £",
               "LayHedgePctDisplay":"Lay Hedge %","RankProb":"RankProb",
               "RankEdge":"RankEdge"}
    ...
    for c in cols:
        ...
        tree.column(c, width=118 if c not in (
    ...
    for _, r in df.iterrows():
        tree.insert("", tk.END, values=[
            ...
            f"£{r.BackStake:.2f}",
            f"£{r.BackWin:.2f}",
            f"£{r.BackHedgeStake:.2f}",
            r.BackHedgePctDisplay,
            f"£{r.LayStake:.2f}",
            f"£{r.LayLiability:.2f}",
