...
def calculate_score():
    try:
        ...
        shots_diff   = float(shots_diff_entry.get())   # positive = behind, negative = ahead
    except ValueError:
    ...
    # Always laying: penalize by absolute shots difference
    score -= (abs(shots_diff) / math.sqrt(max(holes_left, 1))) * SB_SCALE
...
fields = [
    ...
    "Shots +/-"   # concise label for shots behind/ahead
]
entries = {}
...
finish_frame = tk.Frame(root); finish_frame.grid(row=last5_row, column=1, pady=2)
finish_entries = []
for j in range(5):
